...
def dashboard():
    ...
    # FIX 1: Initialize as DictCursor immediately
    cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
    ...
    for row in rows:
        img_b64 = None
        # FIX 2: Use Dictionary Keys instead of Numbers
        if row['room_image']:
            try:
                img_b64 = base64.b64encode(row['room_image']).decode('utf-8')
            except Exception:
                img_b64 = None

        max_capacity = row['max_people']
        occupied_people = row['occupied_people']
        ...
        rooms.append({
            ...
            'water_bill_included': row['water_bill_included'],
            'light_bill_included': row['light_bill_included'],
    ...
    # --- NOTIFICATION LOGIC (Now safe to use ['count']) ---
    if session.get('is_owner'):
        cursor.execute("SELECT COUNT(id) as count FROM bookings WHERE room_id IN (SELECT id FROM rooms WHERE owner_id = %s) AND request_status = 'pending'", (user_id,))
        res = cursor.fetchone()
        pending_requests_count = res['count'] if res else 0
            
    else:
        # 1. Count for Navbar Badge
        cursor.execute("SELECT COUNT(id) as count FROM bookings WHERE user_id = %s AND request_status = 'approved'", (user_id,))
        res = cursor.fetchone()
        student_notifications_count = res['count'] if res else 0

        # 2. Fetch specific room names
        cursor.execute("""
            SELECT b.id, r.address 
            FROM bookings b 
            JOIN rooms r ON b.room_id = r.id 
            WHERE b.user_id = %s AND b.request_status = 'approved'
        """, (user_id,))
    ...
    show_feedback = False
    # Re-using a new cursor is fine, but we can just use the main logic structure
    ...
    return render_template('dashboard.html', 
                           ...
                           student_notifications_count=student_notifications_count,
...
def room_detail(room_id):
    ...
    if not session.get('is_owner'):
        cursor.execute("""
            SELECT id, request_status, num_people_booking, user_id 
            FROM bookings 
            WHERE user_id = %s AND room_id = %s AND request_status != 'rejected'
...
# app.py - Updated respond_request function
...
def respond_request(booking_id, action):
    ...
    cursor = mysql.connection.cursor()
    # Update the status and set a timestamp for the notification
    cursor.execute("""
        UPDATE bookings b
        JOIN rooms r ON b.room_id = r.id
    ...
    if cursor.rowcount > 0:
        status_msg = "approved! The student will be notified to upload documents." if action == 'approve' else "rejected."
        flash(f'Request {status_msg}', 'success')
    else:
